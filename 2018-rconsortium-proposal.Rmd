...
There are many ways to go from missing data to complete observations.
...
software such as SAS, SPSS, or STATA. To be exact, there are currently 274 R
...
intend to work with is Stefan van Buuren, whose website
...
[Julie Josse](http://juliejosse.com/) is a professor of statistics at Ecole Polytechnique has written R packages for missing data,
include missMDA, and has also authored multiple academic papers on the topic ...
...
4. (M11-M12). Organize data benchmark repository. Start writing an
article (for the R journal) on the experience. We would also at this point like to start considering ideas such as universal approaches to multiple imputation, implementing other special types of missing value (other than NA like STATAs special missing values), altering messages in the base R to encourage other approaches than deleting missing observations by default, or at least indicating the risks, etc.
...
Julie and Nick will jointly supervise a research assistant, or multiple research assistants, who will help with the Milestones desribed.
